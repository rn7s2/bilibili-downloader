...
import { gotBuffer } from './utils/get-buffer'
import { saveDanmukuFile } from './utils/save-danmuku-file'
...
export class JsonDanmaku {
  ...
  async fetchInfo () {
    let viewBuffer: any
    try {
      viewBuffer = await gotBuffer(`https://api.bilibili.com/x/v2/dm/web/view?type=1&oid=${this.cid}`, gotConfig)
    } catch (error) {
    ...
    const segments = await Promise.all(new Array(total).fill(0).map(async (_, index) => {
      let buffer: any
      try {
        buffer = await gotBuffer(`https://api.bilibili.com/x/v2/dm/web/seg.so?type=1&oid=${this.cid}&segment_index=${index + 1}`, gotConfig)
      } catch (error) {
        throw new Error('获取弹幕信息失败')
      }
      if (!buffer) {
...
export const getUserDanmakuConfig = async (title: string) => {
  ...
  // title 不为空，应该不会出现下述情况
  // for (const [key, value] of Object.entries(config)) {
  //   if (value === undefined || value === null) {
  //     console.warn('danmaku config invalid for key', key, ', value =', value)
  //     config[key as keyof typeof config] = defaultConfig[value as keyof typeof defaultConfig]
  //   }
  // }
  return config
}
